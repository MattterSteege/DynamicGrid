 ...
 * @constructor
 */class dateTypePlugin extends TypePlugin{constructor(){super(),this.operators=["==","!=",">","<",">=","<=","in","><"],this.dateFormat="d-m-yyyy"}validate(e){if(null==e)return!1;if("string"==typeof e)return this.parseValue(e)instanceof Date;if(e instanceof Date)throw new GridError("Date values should be provided as strings in d-m-yyyy format, not as Date objects.");return!1}parseValue(e){if(null==e)return null;if("string"==typeof e){const t=e.split("-");if(3!==t.length)throw new Error("Invalid date format, expected d-m-yyyy");const n=parseInt(t[0],10),r=parseInt(t[1],10),o=parseInt(t[2],10);if(isNaN(n)||isNaN(r)||isNaN(o))throw new Error("Invalid date format, expected d-m-yyyy");return n>=1e3?new Date(n,r-1,o):new Date(o,r-1,n)}throw new Error("Invalid date value: "+e+". Date values should be provided as strings in [d]d-[m]m-yyyy format.")}evaluate(e,t,n,r){if(t&&r&&t.size<=r.size)for(const n of t.keys())this.evaluateCondition(n,e.operator,e.value)||t.get(n).forEach((e=>r.delete(e)));else for(const t of r)this.evaluateCondition(n[t][e.field],e.operator,e.value)||r.delete(t);return r}evaluateCondition(e,t,n){if("in"===t&&(n=JSON.parse(n)),Array.isArray(n)&&n.length>0&&"in"===t)return n.includes(e.toString());const r=this.parseValue(e),o=this.parseValue(n);switch(t){case"==":return r.toString()===o.toString();case"!=":return r.toString()!==o.toString();case">":return r.toDate()>o.toDate();case"<":return r.toDate()<o.toDate();case">=":return r.toDate()>=o.toDate();case"<=":return r.toDate()<=o.toDate();case"><":if(!Array.isArray(o))throw new Error("between operator requires two dates");return r.toDate()>=o[0].toDate()&&r.toDate()<=o[1].toDate()}}renderCell(e){const t=document.createElement("td");return null==e?(t.innerText="",t):(t.textContent=e.toString(),t)}renderEditableCell(e,t){e=this.parseValue(e);const n=document.createElement("td"),r=document.createElement("input");return r.type="date",r.value=e.getFullYear()+"-"+(e.getMonth()+1).toString().padStart(2,"0")+"-"+e.getDate().toString().padStart(2,"0"),r.placeholder=this.dateFormat,r.addEventListener("change",(e=>{try{console.log(e.target.value);const n=this.parseValue(e.target.value);t(n.getDate()+"-"+(n.getMonth()+1)+"-"+n.getFullYear())}catch(e){console.error("Invalid date format:",e)}})),n.appendChild(r),n}}
 ...
 * @license MIT
 * @author Matt ter Steege (Kronk)
 */class KeyboardShortcuts{constructor(){this.shortcuts=new Map,this.listener=this.#f.bind(this),document.addEventListener("keydown",this.listener)}#b(e){return e.toLowerCase().replace(/\s+/g,"")}#f(e){const t=this.#b(`${e.ctrlKey?"ctrl+":""}${e.shiftKey?"shift+":""}${e.altKey?"alt+":""}${e.metaKey?"meta+":""}${e.key}`),n=this.shortcuts.get(t);n&&(e.preventDefault(),n(e))}addShortcut(e,t,n){const r=this.#b(e);this.shortcuts.has(r)?console.warn(`Shortcut '${e}' is already assigned.`):this.shortcuts.set(r,n)}removeShortcut(e){const t=this.#b(e);this.shortcuts.has(t)?this.shortcuts.delete(t):console.warn(`Shortcut '${e}' does not exist.`)}listShortcuts(){return Array.from(this.shortcuts.keys())}clearShortcuts(){this.shortcuts.clear()}destroy(){document.removeEventListener("keydown",this.listener),this.clearShortcuts()}}
